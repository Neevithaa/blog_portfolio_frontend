...
function Blogs() {
    ...
    useEffect(() => {
        window.scrollTo(0, 0);
        axios.get("https://blog-portfolio-2ein.onrender.com/api/blogs").then((res) => {
         ...
         auth.onAuthStateChanged( function(user){
          if(user)
          {
             if(user.uid=='ESQW6n1c93U2sOrLXxw1UNBdAxW2')
             {
                 setadmin(true)
             }
             else{
    ...
    const handleLike = async (blog_id) => {
        try {
            const response = await axios.patch(`https://blog-portfolio-2ein.onrender.com/api/blogs/like/${blog_id}`);
    ...
    const handleNewBlogSubmit = (event) => {
        ...
        const likes = 0
        axios.post("https://blog-portfolio-2ein.onrender.com/api/blogs", { newTitle, date, newContent, likes }).then((res) => {
            console.log(res.data)
